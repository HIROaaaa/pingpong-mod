...
function paddleAtlas() {
  /*
   * 【2026-09-22 修：贴图尺寸与模型 UV 对不上，拍面看着是木色】
   * 原来这里是 `new Canvas(64, 32)`，而 `save()` 的默认高度取的是 `this.size`（= 64），
   * 所以画布被**拉伸保存成 64×64**：真正画了内容的只有左上 64×32，下半 32 行是透明。
   *
   * 而模型（pingpong_paddle_v7.json，32 个元素）用的 UV 是 **8 单位一格**的老写法
   * （[0,0,8,8] 红 / [8,0,16,8] 黑 / [0,8,8,16] 木芯 / [8,8,16,16] 木侧），
   * 在 64×64 的贴图上每格只有 8px —— 拍面（直径 13px）实际只采到左上角一小块 16×16 里的
   * 深色边界皮肤，于是红黑胶皮都看不出来，整体呈现木色。（用户反馈"材质加载不出来"的实况。）
   *
   * 现在把贴图做成与 UV **严格对应**的 32×32：四个 16×16 象限各自铺满一种材质，
   * 模型 UV 的每 8 单位正好等于一个象限的一半（8px），采样点稳稳落在材质内部。
   *
   * ```
   *   0,0  ┌────────────┬────────────┐
   *        │ 红胶皮(面) │ 黑胶皮(面) │   uv [0,0,8,8] / [8,0,16,8]
   *   16,0 ├────────────┼────────────┤
   *        │ 木芯(柄)   │ 木芯(侧面) │   uv [0,8,8,16] / [8,8,16,16]
   *        └────────────┴────────────┘
   * ```
   */
  const c = new Canvas(32);
  c.blit(rubberPatch(RUBBER, RUBBER_DARK), 0, 0);              // 左上 16×16 正面红胶皮
  c.blit(rubberPatch([28, 28, 30], [52, 52, 56]), 16, 0);      // 右上 16×16 反面黑胶皮
  c.blit(woodPatch(WOOD, WOOD_DARK, WOOD_LIGHT), 0, 16);       // 左下 16×16 木芯（柄）
  c.blit(woodPatch(WOOD_DARK, [66, 44, 22], WOOD), 16, 16);    // 右下 16×16 木芯（侧面）
  return c;
}
